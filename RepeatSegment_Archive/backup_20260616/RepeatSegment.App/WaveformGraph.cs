...
/// <summary>
/// SkiaSharp-based waveform control that mirrors Python's matplotlib waveform.
/// Shows a sliding window of ~25 seconds around the current playback position,
/// with segment boundaries, repeat highlight, position cursor, and time ruler.
/// Supports mouse drag to manually select a segment.
/// </summary>
public class WaveformGraph : SKElement
{
    ...
    // Window width in seconds (Python: DEFAULT_XWINDOW = 25)
    ...
    private static readonly SKColor DragOverlay = new(0xFF, 0xD7, 0x00, 0x60); // gold 38% opaque
    ...
    private double _dragVisLeft, _dragVisRight, _dragW; // visual window during drag

    /// <summary>Fires when user finishes mouse-drag selection. Args: (t1_seconds, t2_seconds).</summary>
    public event Action<double, double>? SegmentSelected;

    public float[]? SamplesSmall
    {
        get => _samplesSmall;
        set { _samplesSmall = value; InvalidateVisual(); }
    }

    public int SampleRateSmall
    {
        get => _sampleRateSmall;
        set { _sampleRateSmall = value; InvalidateVisual(); }
    }

    public double DurationSeconds
    {
        get => _durationSeconds;
        set { _durationSeconds = value; InvalidateVisual(); }
    }
    ...
    public List<(double T1, double T2)> Fragments
    {
        get => _fragments;
        set { _fragments = value ?? new(); InvalidateVisual(); }
    }

    public int CurrentCounter
    {
        get => _currentCounter;
        set { _currentCounter = value; InvalidateVisual(); }
    }

    public bool RepeatSegment
    {
        get => _repeatSegment;
        set { _repeatSegment = value; InvalidateVisual(); }
    }

    public bool PlayGoMode
    {
        get => _playGoMode;
        set { _playGoMode = value; InvalidateVisual(); }
    }

    public bool IsDarkTheme
    {
        get => _isDarkTheme;
        set { _isDarkTheme = value; InvalidateVisual(); }
    }
    ...
    // Height for time ruler area at bottom
    private const float RulerHeight = 22f;

    // ── Mouse to Seconds conversion ──────────────────────────────────
    ...
    protected override void OnPaintSurface(SKPaintSurfaceEventArgs e)
    {
        ...
        if (_samplesSmall == null || _samplesSmall.Length == 0)
            return;
        ...
        if (visRight > duration)
        {
            visLeft -= (visRight - duration);
            visRight = duration;
            if (visLeft < 0) visLeft = 0;
        }
        ...
        DrawDragOverlay(canvas, w, waveAreaH, visLeft, visRight);
    ...
    private void DrawDragOverlay(SKCanvas canvas, float w, float h,
        double visLeft, double visRight)
    {
        if (!_isDragging) return;
        ...
        canvas.DrawRect((float)x1, 0, (float)(x2 - x1), h, paint);
    }

    private void DrawSegmentHighlight(SKCanvas canvas, float w, float h,
        double visLeft, double visRight)
    {
        if (_fragments.Count == 0 || _currentCounter >= _fragments.Count)
            return;
        ...
        var paint = new SKPaint
        {
            Color = _isDarkTheme ? HighlightFillDark : HighlightFill,
            Style = SKPaintStyle.Fill,
        };
        canvas.DrawRect((float)x1, 0, (float)(x2 - x1), h, paint);
    }

    private void DrawSegmentLines(SKCanvas canvas, float w, float h,
        double visLeft, double visRight)
    {
        ...
        {
            if (t1 >= visLeft && t1 <= visRight)
            {
                float x = (float)((t1 - visLeft) / (visRight - visLeft) * w);
                canvas.DrawLine(x, 0, x, h, paint);
            }
            if (t2 >= visLeft && t2 <= visRight)
            {
                float x = (float)((t2 - visLeft) / (visRight - visLeft) * w);
                canvas.DrawLine(x, 0, x, h, paint);
            }
        }
    }

    private void DrawWaveform(SKCanvas canvas, float w, float h,
        double visLeft, double visRight)
    {
        if (_samplesSmall == null || _samplesSmall.Length == 0)
            return;
        ...
        var paint = new SKPaint
        {
            Color = _isDarkTheme ? WaveColorDark : WaveColor,
            StrokeWidth = 1,
            Style = SKPaintStyle.Stroke,
            IsAntialias = true,
        };
    ...
    private void DrawCursor(SKCanvas canvas, float w, float h,
        double pos, double visLeft, double visRight)
    {
        double visWidth = visRight - visLeft;
        if (visWidth <= 0) return;
        ...
        canvas.DrawLine(cursorX, 0, cursorX, h, paint);
    }

    private void DrawTimeRuler(SKCanvas canvas, float w, float h,
        double visLeft, double visRight)
    {
        double visWidth = visRight - visLeft;
        if (visWidth <= 0) return;
        ...
        var paint = new SKPaint
        {
            Color = _isDarkTheme ? new SKColor(0xCC, 0xCC, 0xCC) : new SKColor(0x55, 0x55, 0x55),
            StrokeWidth = 1.5f,
            Style = SKPaintStyle.Stroke,
            IsAntialias = true,
        };
        ...
        {
            Color = _isDarkTheme ? new SKColor(0xEE, 0xEE, 0xEE) : new SKColor(0x22, 0x22, 0x22),
            TextSize = 16,
            IsAntialias = true,
        ...
        for (double t = firstTick; t <= visRight; t += 5.0)
        {
            ...
            int mins = (int)(t / 60);
            int secs = (int)(t % 60);
            string label = $"{mins}:{secs:D2}";
            canvas.DrawText(label, x + 2, h - 1, textPaint);
        }

        var subPaint = new SKPaint
        {
            Color = _isDarkTheme ? new SKColor(0x88, 0x88, 0x88) : new SKColor(0x99, 0x99, 0x99),
            StrokeWidth = 1f,
            Style = SKPaintStyle.Stroke,
        };
